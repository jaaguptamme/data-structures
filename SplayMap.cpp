...
using namespace std;
template <typename T,typename V> struct SplayTree {
	struct Node {
		Node(){}
		Node(T key_arg,V value_arg){
            key=key_arg;
            value=value_arg;
		}
		T key; //!< Value associated with node
		V value;
	...
	struct iterator {
		...
		const T &operator*() { return node->key; }
		Node *node;
	...
	Node *find(const T &key) {
		auto x = root;
		while(x && key != x->key){
			const auto next = x->child[key > x->key];
			if(!next){
				x->splay();
			}
			x = next;
